# Flash Feather Framework v2.1 - Complete Documentation
...
class AuthMiddleware(BaseHTTPMiddleware):
    def __init__(self, app, **kwargs):
        ...
        self.access_token_expire = int(os.getenv("ACCESS_TOKEN_EXPIRE_SECONDS", "900"))
        self.refresh_token_expire = int(os.getenv("REFRESH_TOKEN_EXPIRE_SECONDS", "604800"))
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        access_token = request.cookies.get("access_token") or self.get_header_token(request)
        ...
        if access_token:
            user_data = await AuthService.validate_and_refresh_token(
                access_token, refresh_token, from_cookie=bool(request.cookies.get("access_token"))
            )

        # If access token is None/expired but we have refresh token, try refresh-only
        elif refresh_token:
...
**`app/services/auth_service/auth_service.py`**
...
**`app/services/auth_service/google_auth_service.py`**
...
if not GOOGLE_CLIENT_ID or not GOOGLE_CLIENT_SECRET:
    print("Warning: Google OAuth credentials not found. Google sign-in will not work.")
...
**`app/routes/api/auth/auth_api.py`**
...
async def register(user_data: RegisterRequest, response: Response, db: Session = Depends(get_db)):
    ...
    return AuthResponse(message="Registration successful", user=UserResponse.model_validate(user))
...
block module_imports %} import '/static/js/components/global/app_header.js'; {%
  ...
  <div class="hero">
    ...
    <div class="features">
      <div class="feature">
        ...
        <p>Built on FastAPI with optimized performance</p>
      </div>
      <div class="feature">
        <i class="fas fa-lock"></i>
        <h3>Secure Auth</h3>
        <p>JWT with HTTP-only cookies and Google OAuth</p>
      </div>
      <div class="feature">
        <i class="fas fa-cloud"></i>
        <h3>Production Ready</h3>
        <p>Environment-aware with automatic deployments</p>
      </div>
    </div>
...
class AppHeader extends LitElement {
  ...
  async loadCurrentUser() {
    try {
      // Frontend only asks server for what it needs - no auth logic
      const response = await fetch("/api/auth/me");

      if (response.ok) {
        this.user = await response.json();
      }
      // If not authenticated, user stays null - server handles auth
    } catch (error) {
...
Flash Feather v2.1 provides a complete, modern web framework with:

- ✅ **Complete authentication system** with JWT and Google OAuth
